...
con=sq.connect(host='localhost',user='root', passwd='Simba@666',\
                database='mark3',charset='utf8')

cap=float(input("Enter your capital for investment : "))

time_input=float(input("Enter the time of investment in stock market\
...
for i,j in Df5.iterrows():
   x=j[time_input-0.1-0.4*(time_input==int(time_input))]
   lst+=[float(x)]
...
'''Df5=pd.DataFrame(1st,index=stocks,columns=[time_input])
print(Df5)
Df6=(2*Df4-Df5)/2 #Predicted value
difference=Df6-Df3 #DataFrame storing difference in avg and predicted_value
'''
status=[]
for k in range(n1-1,171,c): #Jumps of 9 units to ensure correct status
    z=data[k][3] #of each share corresponding to a particular time
    status+=[z]

Df7=pd.DataFrame([lst_predicted,status] ,columns=stocks,index=['price','Status'])
Df8=Df7.T  #Status of each stock
lst3=[]

for i in Df8.iterrows(): #NEW price correcponding to status
    if i[1][1]=='o':
        j=0.99*float(i[1][0])
        lst3+=[np.round(j,2)]
    if i[1][1]=='n':
    ...
    if i[1][1]=='u':
        ...
        lst3+=[np.round(j,2)]
#print(lst3)
Df_penultimate=pd.DataFrame([((np.array(lst3)-np.array(lst1))/(np.array(lst1))),status],\
                            columns=stocks,index=['price','Status'])
...
den=count/3+(3-count)*2/3  #2:1 u/n:o
num=count/3
fraction_o=num/den
if len(stock_o)!=0:
...
for i in range(9):
    ...
    for j in range(len (stock_n)):
        if Df_plot.columns[i]==stock_n[j]:
            ...
            #pl.legend([Df_plot.columns[i]],loc="lower right")
            #print(b,'\n',a,'\n',colour_str)
            #pl.plot(b,list(a.T),color=colour_str)
            #a.plot(kind='line')
            #pl.plot(a,color=colour_str)
            #pl.show()
            pl.plot(a,color=colour_str)
